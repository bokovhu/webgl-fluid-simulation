export default class WasmMarcher {
    ...
    constructor (gl, wasmModule) {
        ...
        this.wasmCalculate = this.wasmModule.cwrap (
            'wasmCubeMarch_calculate',
            'number',
            [
                // float* outVertices
                'number',
    ...
    calculate (
        ...
        let numVertices = this.wasmCalculate (
            ...
            vertexOffsetX, vertexOffsetY, vertexOffsetZ,
            scaleX, scaleY, scaleZ,
            isoLevel
        )

        // Copy results back to output vertices
        outVertices.set (this.wasmModule.HEAPF32.slice (this.wasmOutputVerticesIndex, this.wasmOutputVerticesIndex + (6 * numVertices)))
